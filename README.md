...
Перед запуском теста необходимо собрать Docker-образ Kafka Connect с необходимыми плагинами:

```bash
./build-kafka-connect-debezium-jdbc.sh
```

**Dockerfile** (`connect/Dockerfile`) разворачивает образ на базе `debezium/connect:1.9` и добавляет:
- **JDBC Sink Connector** (kafka-connect-jdbc-10.2.0.jar) — для записи данных в PostgreSQL
- **PostgreSQL JDBC Driver** (postgresql-42.6.0.jar) — для подключения к БД
...
- **Отказ целого ДЦ (DC2)**: приводит к ошибкам в Kafka Connect
- **Причина**: при RF=3 и потере DC2 остаётся только 1 реплика в DC1 (при условии распределения реплик по ДЦ). Это меньше MinISR=2, поэтому часть партиций становится недоступной для записи — продюсеры не могут записывать данные, пока не восстановится недостающая реплика

**Вывод**: RF=3 с MinISR=2 недостаточно для отказоустойчивости при потере целого дата-центра.
...
Даже при корректно настроенных RF и MinISR, при потере связи с мастером PostgreSQL коннекторы Debezium (source) и JDBC Sink перейдут в состояние FAILED, так как не смогут подключиться к базе данных. В данной тестовой схеме все воркеры Kafka Connect должны находиться в том же дата-центре, где находится мастер БД, иначе недоступность БД сделает коннекторы неработоспособными независимо от состояния Kafka кластера.
